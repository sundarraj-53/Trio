...
public class home extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(role==0 && role2==0 && store.getAdmin().equals("true")){
            bottomNavigationView.getMenu().findItem(R.id.post).setVisible(true);
            bottomNavigationView.getMenu().findItem(R.id.blood_trio).setVisible(false);
        }
        bottomNavigationView.setOnItemSelectedListener(new NavigationBarView.OnItemSelectedListener() {
            ...
            public boolean onNavigationItemSelected(@NonNull MenuItem item) {
                ...
                if(item.getItemId()==NEW_POST_ID){
                    getSupportFragmentManager().beginTransaction().replace(R.id.container,work).commit();
                    return true;
                }
